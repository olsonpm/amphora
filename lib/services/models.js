  ...
  if (executeRender) {
    ...
    promise = bluebird.try(() => {
      ...
      if (ms > timeoutLimit * 0.5) {
        let slowMsg = ''
          + `slow get ${uri} ${ms}ms`
          + `\n  db.get:  ${times.afterGet - times.start}ms`
          + `\n  upgrade: ${times.afterUpgrade - times.afterGet}ms`
          + `\n  render:  ${times.afterRender - times.afterUpgrade}ms`;

        if (locals.amphoraRenderTimes.length) {
          const timesDuringRender = locals.amphoraRenderTimes
            .map(({ data, label, ms }) => {
              let msg = `  ${label}: ${ms}ms`;
