    ...
    promise = bluebird.try(() => {
      ...
      if (ms > timeoutLimit * 0.5) {
        const dbDuration = times.afterGet - times.start,
          upgradeDuration = times.afterUpgrade - times.afterGet,
          renderDuration = times.afterRender - times.afterUpgrade,
          detailedRenderTimes = _.get(locals, 'amphoraRenderTimes', []);
